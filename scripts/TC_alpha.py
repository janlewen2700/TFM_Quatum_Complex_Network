...
def plot_combined_figure(logsweep="../logs/log.csv", logsize="../logs/log.csv", logk="../logs/log.csv", savefig=None):
    ...
    plot_heatmap_from_log(logfile=logsweep, value_col="r_mean", name="SD", N=[4000], x_col="beta", y_col="gamma", ax=ax1)
    ax1.text(-0.17, 1.27, "(a)", transform=ax1.transAxes, fontweight='bold')

    # 3. Call mean/scaling plot (Subplot B)
    ax2 = fig.add_subplot(gs[1, :])
    plot_mean_from_log(logfile=logsize, parameterx="beta", parametery="r_mean", parameter_control = "gamma", control_val = 6.01, N_groups=[500, 1000, 2000, 4000, 6000, 8000], rel_tol=0.10, name="SD", ax=ax2)
    ax2.text(-0.17, 1.07, "(b)", transform=ax2.transAxes, fontweight='bold')
    
    ax3 = fig.add_subplot(gs[2, :])
    plot_mean_by_k_fixed_N(logfile=logk, parameterx="beta", parametery="r_mean", parameter_control = "gamma", control_val = 6.01, target_N=2000, rel_tol=0.10, name="SD", ax=ax3)
...
plot_combined_figure(logsweep="../logs/log_sweep.csv", logsize="../logs/log_sizes.csv", logk="../logs/log_k.csv", savefig="../figures/heatmap/heat_r_size.pdf")
